...
playerLeft.src = 'fish_red_left.png';
const playerRight = new Image();
playerRight.src = 'fish_red_right.png';
class Player {
  constructor(){
    this.x = canvas.width;
...
function handleBubbles(){
  ...
  for (let i = 0; i < bubbleArr.length;i++){
    if (bubbleArr[i].y < 0 - bubbleArr[i].radius * 2){
      bubbleArr.splice(i, 1);
    }
    if (bubbleArr[i]) {
      if (bubbleArr[i].distance < bubbleArr[i].radius + player.radius){
        if (!bubbleArr[i].counted){
          if (bubbleArr[i].sound == 'sound1'){
            bubblePop1.play();
          } else {
            bubblePop2.play();
          }
          score++;
          bubbleArr[i].counted = true;
          bubbleArr.splice(i, 1);
        }
      }
    }
